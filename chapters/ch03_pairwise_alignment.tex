...
BLAST itself \cite{altschul1990} is the most influential algorithm in computational biology by the usual yardsticks: citations, daily use, and scientific impact. Its design repeats a pattern common in bioinformatics: exact algorithms exist (Smith--Waterman) but are too slow for the scale of modern databases, so heuristic approximations that sacrifice a small amount of sensitivity for orders-of-magnitude speedups become the practical standard. The success of BLAST rests on algorithmic cleverness (the word-hit seeding strategy) and on the statistical foundation from Karlin and Altschul, which lets users interpret results with clear E-values.
