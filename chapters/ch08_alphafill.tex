...
\end{equation}
where $\vr_i^{\text{AF}}$ and $\vr_i^{\text{PDB}}$ are the C$_\alpha$ positions of residue $i$ in the AlphaFold model and the PDB structure, respectively, and $N$ is the number of aligned residues. The optimal rotation is found by the Kabsch algorithm, which solves a singular value decomposition of the cross-covariance matrix of the two coordinate sets.
