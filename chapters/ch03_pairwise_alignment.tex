...
\newthought{Needleman and Wunsch} published the first dynamic programming algorithm for sequence alignment in 1970 \cite{needleman1970}. The formulation we present here uses the streamlined recursion due to Gotoh and others, which is the version taught in most textbooks.
...
\newthought{The time complexity} of Needleman--Wunsch is $O(mn)$: we fill an $m \times n$ matrix, and each cell requires constant-time computation. The space complexity is also $O(mn)$ for the full matrix, though Hirschberg's divide-and-conquer technique can reduce space to $O(\min(m,n))$ while maintaining $O(mn)$ time.
...
It is important to understand what the Karlin--Altschul theory does and does not tell us. It tells us the distribution of alignment scores under the null hypothesis of random, unrelated sequences. It does \emph{not} tell us the probability that two sequences are homologous given a high score. That is a question of Bayesian inference, requiring a prior on the fraction of database sequences that are truly related to the query. In practice, most biologists treat low E-values (say, below $10^{-5}$) as strong evidence of homology and high E-values (above 1) as absence of evidence, with a ``twilight zone'' in between where additional information (structural similarity, conserved motifs, phylogenetic context) is needed.
