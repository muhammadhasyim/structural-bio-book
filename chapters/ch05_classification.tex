...
logistical: how should one organize this growing collection? Proteins plainly fell
...
of Proteins (SCOP) database in 1995. Its organizing principle was evolutionary:
...
relationships simultaneously. But the curation bottleneck persisted.
...
$\beta$-strands, computed automatically from atomic coordinates using DSSP or
...
a threshold in SSAP were grouped together.
...
most assignments but disagree on enough cases (roughly 20\% of domain
superfamilies, by various estimates) to remind us that protein classification is
...
coordinate sets.\sidenote{The Kabsch algorithm computes the $3\times 3$ matrix
...
\newthought{Dali}, developed by Liisa Holm in the early 1990s, compares proteins
...
contact map: it looks for clusters of residues that are in close spatial contact
...
scale of the entire PDB.\sidenote{Domain boundaries predicted by automated methods
...
\newthought{Gene3D extends} domain classification to the genome scale. Starting
...
this challenge. TED takes AlphaFold-predicted structures, parses them into domains
...
arbitrary.\sidenote{Kolodny and colleagues showed in 2006 that if one embeds SCOP
domains in a high-dimensional space using pairwise structural-similarity scores,
the resulting cloud of points does not form well-separated clusters. Fold space
...
The SCOP database was described by Murzin, Brenner, Hubbard, and Chothia in 1995.
...
tree, was described by Andreeva et al.\ in 2014 and is maintained at
\texttt{scop.berkeley.edu}. CATH was introduced by Orengo et al.\ in
...
empirically against SCOP classifications by Xu and Zhang in 2010, who showed that
...
The Kabsch algorithm for optimal superposition was published in 1976 and remains
the standard method. Equivalent formulations using quaternions (by Horn, 1987) or
...
Dali was introduced by Holm and Sander in 1993. The original FSSP database is no
...
newly determined structure. SSAP was first described by Taylor and Orengo in 1989
and refined in subsequent papers; its integration into the CATH pipeline is
described in the 1997 CATH paper.

InterPro, first described by Mulder et al.\ in 2003, is maintained by the European
...
The Encyclopedia of Domains project was described by Sherborne et al.\ in 2024.
...
Koehl, and Levitt (2005), who mapped SCOP domains into a continuous
