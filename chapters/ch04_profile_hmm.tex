...
In 1994, Anders Krogh, Michael Brown, I.\ Saira Mian, Kimmen Sj\"olander, and David Haussler published a paper in the \textit{Journal of Molecular Biology} that changed the way bioinformaticians think about protein families~\cite{krogh1994}.\sidenote{A.\ Krogh, M.\ Brown, I.\ S.\ Mian, K.\ Sj\"olander, and D.\ Haussler. Hidden Markov models in computational biology: Applications to protein modeling. \textit{J.\ Mol.\ Biol.}, 235(5):1501--1531, 1994.}
...
Some positions are nearly invariant across all family members, often because they form the catalytic site; other positions accept almost any amino acid.
...
\begin{figure}
  ...
  \caption{Clustal Omega alignment of three Parkin orthologs (\textit{Drosophila}, human, mouse). Conserved positions are highlighted; dashes indicate gaps.~\cite{sievers2011}}
...
The workhorse approximation is progressive alignment, the strategy behind CLUSTAL~W~\cite{thompson1994} and its successor CLUSTAL~$\Omega$~\cite{sievers2011}.
...
Third, align the sequences following the guide tree from leaves to root: align the two closest sequences first, then successively align each next-closest sequence or group to the growing alignment.
...
The architecture was introduced by Krogh et al.\ in 1994~\cite{krogh1994} and refined by Eddy and others over the following decade~\cite{durbin1998}.
...
Farrar's 2007 striped Smith--Waterman algorithm~\cite{farrar2007} showed that dynamic programming over a scoring profile can be parallelized across multiple target residues using SIMD registers.
...
The hidden Markov model formalism was developed by Leonard Baum and colleagues at the Institute for Defense Analyses in the late 1960s, originally for classified work on signal processing~\cite{baum1970}.
...
The application to biological sequences began in the early 1990s with work by Gary Churchill on multiple alignment~\cite{churchill1989} and by Krogh, Mian, and Haussler on gene finding~\cite{krogh1994hmmgene}.
The Krogh et al.\ 1994 paper on profile HMMs for protein families was a landmark that connected the statistical learning perspective with the biological problem of protein family classification~\cite{krogh1994}.
...
The key technical contributions were the MSV filter, the use of SIMD vectorization following Farrar's striped algorithm~\cite{farrar2007}, and the wing-folding trick for efficient local alignment.
...
Gribskov, McLachlan, and Eisenberg introduced sequence profiles in 1987~\cite{gribskov1987}, and Henikoff and Henikoff developed the Blocks database of ungapped local alignments that was used to train substitution matrices and Dirichlet priors~\cite{henikoff1992}.
...
The use of Bayesian priors for estimating amino acid frequencies from small samples is one of the most successful applications of Bayesian statistics in computational biology, and the Dirichlet mixture approach of Sj\"olander et al.~\cite{sjolander1996} remains the standard in HMMER and related tools.
...
Thompson et al.'s CLUSTAL~W~\cite{thompson1994} was the most cited bioinformatics paper for many years, and T-Coffee~\cite{notredame2000} introduced the consistency principle that influenced subsequent methods including PROBCONS, MUSCLE, and MAFFT.
