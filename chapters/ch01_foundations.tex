...
\newthought{The peptide bond is planar.}  Linus Pauling recognized in the 1930s that resonance between the C$'$=O and C$'$--N bonds gives the peptide linkage partial double-bond character~\cite{pauling1960}.\sidenote{The C$'$--N bond length in a peptide is about 1.33~\AA, intermediate between a single bond (1.47~\AA) and a double bond (1.25~\AA).  This partial double-bond character restricts rotation.}  The six atoms of the peptide unit (C$_\alpha^{(i)}$, C$'$, O, N, H, C$_\alpha^{(i+1)}$) lie approximately in a plane, and the dihedral angle $\omega$ about the C$'$--N bond is locked near $180^\circ$ (\textit{trans}) for most residues.  The \textit{cis} configuration ($\omega \approx 0^\circ$) occurs occasionally before proline, where the five-membered pyrrolidine ring reduces the energy difference between the two isomers.
...
Linderstrom-Lang introduced, in 1952, the classification of protein structure into primary, secondary, tertiary, and quaternary levels~\cite{linderstrom2lang1952}.  This hierarchy remains the standard vocabulary of the field, though we now understand that the boundaries between levels are not always sharp.
...
The hydrophobic effect is the dominant driving force for protein folding~\cite{kauzmann1959,dill1990}.  When a nonpolar solute is transferred from a nonpolar environment into water, the free energy change is positive (unfavorable), even though the enthalpy change can be slightly negative (favorable, due to van~der~Waals contacts with water molecules).  The explanation lies in entropy.
...
\begin{equation}
  ...
  \label{eq:karplus}
\end{equation}
where $A$, $B$, and $C$ are empirically determined parameters (approximately 6.51, $-1.76$, and 1.60~Hz for backbone $^3J_{\text{HN-H}\alpha}$)~\cite{karplus1959}.  Measured $J$-couplings thus constrain the backbone conformation.
...
\begin{equation}
  ...
  \label{eq:fsc}
\end{equation}
where $F_1$ and $F_2$ are the Fourier transforms of the two half-maps and the sums run over all voxels in a thin spherical shell at spatial frequency $s$.  The resolution is reported as the spatial frequency at which the FSC drops to 0.143, a threshold derived from the signal-to-noise ratio needed for reliable phase information~\cite{rosenthal2003}.\sidenote{The FSC = 0.143 criterion, proposed by Rosenthal and Henderson (2003), corresponds to the spatial frequency at which the signal is half the noise in a single reconstruction.  Other thresholds (0.5, 0.3) are occasionally used; the choice of threshold can shift the reported resolution by 0.5--1.0~\AA.}
...
The history of protein crystallography is told vividly in Judson's \textit{The Eighth Day of Creation} (1979) and in Perutz's own essays collected in \textit{I Wish I'd Made You Angry Earlier} (1998).  The original Cavendish myoglobin and hemoglobin papers are cited in the main text~\cite{kendrew1958,kendrew1960,perutz1960}.
