...
# Substitution Matrices {#ch03-pairwise-alignment-sec:substitution-matrices}
...
## The PAM Matrices {#ch03-pairwise-alignment-sec:pam}

**Dayhoff's approach** was phylogenetic [@dayhoff1978]. She collected 71 groups of closely related proteins, each group containing sequences that were at least 85% identical. At this level of similarity, the correct alignment is unambiguous and can be constructed by inspection. Within each group, she inferred a phylogenetic tree and mapped every amino acid substitution onto a branch of that tree. The total catalog contained 1,572 observed substitutions.

Let us formalize her construction. We have 20 amino acids indexed by $a, b \in \{1, \ldots, 20\}$. Let $n_{ab}$ denote the number of times amino acid $a$ was observed to mutate into amino acid $b$ (or $b$ into $a$) across all branches of all trees. Because Dayhoff could not determine the direction of change on an unrooted tree, the raw counts are symmetrized: $$\begin{equation}
  \label{eq:pam-raw-counts}
  n_{ab} = n_{ba}.
\end{equation}$$

We want a $20 \times 20$ mutation probability matrix $\mathbf{M}$ where entry $M_{ab}$ gives the probability that amino acid $a$ is replaced by amino acid $b$ in one "PAM unit" of evolutionary time.[^ch03-pairwise-alignment-2] To compute $M_{ab}$ from the raw counts, we need the *mutability* of each amino acid. Define $$\begin{equation}
  \label{eq:mutability}
  m_a = \frac{\sum_{b \neq a} n_{ab}}{f_a \cdot N},
\end{equation}$$ where $f_a$ is the background frequency of amino acid $a$ in the dataset, and $N$ is the total number of aligned positions examined. The mutability $m_a$ measures how often amino acid $a$ changes relative to how often it appears.

The off-diagonal entries of $\mathbf{M}$ are then $$\begin{equation}
  \label{eq:pam-off-diag}
  M_{ab} = \lambda \, m_a \, \frac{n_{ab}}{\sum_{c \neq a} n_{ac}}, \quad a \neq b,
\end{equation}$$ where $\lambda$ is a proportionality constant chosen so that the expected number of mutations per position equals exactly 0.01 (that is, 1% divergence). The diagonal entries are fixed by the requirement that each row sums to one: $$\begin{equation}
  \label{eq:pam-diag}
  M_{aa} = 1 - \lambda \, m_a.
\end{equation}$$

The constraint that defines one PAM unit is $$\begin{equation}
  \label{eq:pam-constraint}
  \sum_{a=1}^{20} f_a \left(1 - M_{aa}\right) = 0.01,
\end{equation}$$ which simply says that the overall fraction of positions expected to change is 1%. Substituting Equation [\[eq:pam-diag\]](#eq:pam-diag){reference-type="eqref" reference="eq:pam-diag"} into this constraint and solving for $\lambda$ gives $$\begin{equation}
  \label{eq:pam-lambda}
  \lambda = \frac{0.01}{\sum_{a=1}^{20} f_a \, m_a}.
\end{equation}$$

**The matrix $\mathbf{M}$** describes one PAM unit of evolution. To model longer evolutionary distances, we use matrix exponentiation. The PAM-$n$ mutation probability matrix is $$\begin{equation}
  \label{eq:pam-n}
  \mathbf{M}^{(n)} = \mathbf{M}^n.
\end{equation}$$ Entry $M^{(n)}_{ab}$ gives the probability that amino acid $a$ evolves into amino acid $b$ over $n$ PAM units.[^ch03-pairwise-alignment-3]

To convert these probabilities into an alignment scoring matrix, we use log-odds ratios. Define the *target frequency* as $$\begin{equation}
  \label{eq:target-freq}
  q_{ab} = f_a \, M^{(n)}_{ab}.
\end{equation}$$ This is the probability that a random aligned pair consists of $a$ in one sequence and $b$ in the other, under the hypothesis that they share a common ancestor $n$ PAM units ago. Under the null hypothesis of no relationship, the expected frequency of the pair $(a,b)$ is simply $f_a \, f_b$. The log-odds score is $$\begin{equation}
  \label{eq:pam-log-odds}
  s(a,b) = \log_2 \frac{q_{ab}}{f_a \, f_b} = \log_2 \frac{M^{(n)}_{ab}}{f_b}.
\end{equation}$$ The second equality follows from canceling $f_a$. In practice these scores are multiplied by 10 and rounded to integers, yielding the familiar PAM-250 matrix printed in textbooks.[^ch03-pairwise-alignment-4]
...
## The BLOSUM Matrices {#ch03-pairwise-alignment-sec:blosum}

**Henikoff and Henikoff** took a different approach in 1992 [@henikoff1992]. Instead of inferring phylogenetic trees, they worked with the BLOCKS database: a collection of ungapped, multiply aligned segments of protein families. The idea was to count amino acid pair frequencies directly from these conserved blocks, sidestepping the need for evolutionary models altogether.

The construction of BLOSUM-$X$ (where $X$ is a clustering threshold) proceeds as follows. Within each block, sequences that are more than $X$% identical are merged into a single cluster. For BLOSUM-62, this threshold is 62%.[^ch03-pairwise-alignment-5] Each cluster is treated as a single effective sequence, and all pairs of clusters contribute equally to the counts.

Let $f_{ab}$ denote the observed frequency of the pair $(a,b)$ across all columns of all blocks, where $a$ comes from one cluster and $b$ from a different cluster. The matrix $f_{ab}$ is symmetric. The marginal frequencies are $$\begin{equation}
  \label{eq:blosum-marginal}
  p_a = f_{aa} + \frac{1}{2}\sum_{b \neq a} f_{ab}.
\end{equation}$$ The expected pair frequency under independence is $$\begin{equation}
  \label{eq:blosum-expected}
  e_{ab} = \begin{cases} p_a^2 & \text{if } a = b, \\ 2\, p_a \, p_b & \text{if } a \neq b. \end{cases}
\end{equation}$$ The log-odds score is $$\begin{equation}
  \label{eq:blosum-score}
  s(a,b) = \frac{1}{\lambda} \log_2 \frac{f_{ab}}{e_{ab}},
\end{equation}$$ where $\lambda$ is a scaling factor (typically chosen so that the matrix entries are in units of half-bits). In practice, scores are rounded to the nearest integer.

The BLOSUM and PAM matrices are constructed from the same principle (log-odds of observed versus expected pair frequencies) but they differ in their data sources and assumptions. PAM matrices extrapolate from closely related sequences using a Markov model; BLOSUM matrices measure substitution patterns directly at the evolutionary distance implied by the clustering threshold. For database searches, BLOSUM-62 has been found to outperform PAM matrices in most benchmarks, and it remains the default substitution matrix in BLAST [@altschul1997].
...
# Dynamic Programming for Sequence Alignment {#ch03-pairwise-alignment-sec:dynamic-programming}
...
A brute-force enumeration of all possible alignments is out of the question. The number of possible alignments of two sequences of length $m$ and $n$ grows as $\binom{m+n}{m}$, which is exponential in the sequence lengths.[^ch03-pairwise-alignment-6] The key insight, discovered independently by several groups in the late 1960s and 1970s, is that the problem has *optimal substructure*: the best alignment of two sequences can be built from the best alignments of their prefixes. This makes the problem amenable to dynamic programming.

## Global Alignment: The Needleman--Wunsch Algorithm {#ch03-pairwise-alignment-sec:needleman-wunsch}

**Needleman and Wunsch** published the first dynamic programming algorithm for sequence alignment in 1970 [@needleman1970]. The formulation we present here uses the streamlined recursion due to Gotoh and others, which is the version taught in most textbooks.

Define $S_{i,j}$ to be the score of the optimal alignment of the prefixes $x_1 \cdots x_i$ and $y_1 \cdots y_j$. We use a linear gap penalty of $d < 0$ for each gap character. The recurrence is $$\begin{equation}
...
**A worked example** clarifies the procedure. Consider the sequences $\mathbf{x} = \texttt{VDSCCY}$ and $\mathbf{y} = \texttt{VESLCY}$. We use the PAM-250 substitution matrix and a linear gap penalty $d = -8$. The relevant PAM-250 scores (in half-bit units) are:[^ch03-pairwise-alignment-7]

::: {#ch03-pairwise-alignment-tab:pam250-example}
        V      E      S      L      C      Y
  --- ------ ------ ------ ------ ------ ------
   V    4     $-2$   $-1$    2     $-2$   $-2$
   D    2      3      0     $-4$   $-5$   $-4$
  ...
  : Selected PAM-250 scores for the amino acids appearing in our example sequences.
:::
...
::: {#ch03-pairwise-alignment-tab:nw-matrix}
          $-$      V       E       S       L       C        Y
  ----- ------- ------- ------- ------- ------- -------- --------
   $-$     0     $-8$    $-16$   $-24$   $-32$   $-40$    $-48$
    V    $-8$    **4**   $-4$    $-12$   $-20$   $-28$    $-36$
  ...
  : Needleman--Wunsch dynamic programming matrix for `VDSCCY` vs. `VESLCY` with PAM-250 scores and gap penalty $d = -8$. Bold entries indicate the traceback path.
:::
...
## Local Alignment: The Smith--Waterman Algorithm {#ch03-pairwise-alignment-sec:smith-waterman}

Global alignment forces us to align the entire length of both sequences. This is appropriate when comparing two complete proteins of similar length, but it fails badly when we want to find a conserved domain embedded in a longer, unrelated sequence. The solution is *local alignment*, introduced by Smith and Waterman in 1981 [@smith1981].

![A pairwise alignment between two lactotransferrin orthologs, showing the query--match overview track and residue-level alignment with conservation coloring.](../../../figures/ch03_pairwise_alignment.png){#ch03-pairwise-alignment-fig:pairwise-alignment width="\\linewidth"}

The modification is deceptively simple. We add a fourth option to the recurrence: reset the score to zero. $$\begin{equation}
  ...
  H_{i,j} = \max \begin{cases}
    ...
    H_{i,\, j-1} + d.
  \end{cases}
\end{equation}$$ The boundary conditions are $H_{i,0} = 0$ for all $i$ and $H_{0,j} = 0$ for all $j$. These two changes (the zero floor in the recurrence and the zero boundary) mean that the algorithm is free to ignore poorly scoring regions. Any prefix or suffix that would contribute a negative score is simply discarded.

The optimal local alignment score is not found in the bottom-right corner but at the position $(i^*, j^*)$ where $H_{i,j}$ is maximal over the entire matrix: $$\begin{equation}
  \label{eq:sw-max}
  S^* = \max_{i,j} H_{i,j}.
\end{equation}$$ Traceback begins at this maximum and proceeds backward through diagonal, vertical, and horizontal pointers until a cell with $H_{i,j} = 0$ is reached. The resulting path defines the boundaries and content of the local alignment.
...
## Affine Gap Penalties {#ch03-pairwise-alignment-sec:affine-gaps}
...
An *affine gap penalty* captures this intuition: $$\begin{equation}
  \label{eq:affine-gap}
  W(k) = g + k \cdot e,
\end{equation}$$ where $g < 0$ is the gap-opening penalty, $e < 0$ is the gap-extension penalty, $|e| < |g|$, and $k$ is the length of the gap.[^ch03-pairwise-alignment-8]
...
The recurrences are:[^ch03-pairwise-alignment-9] $$\begin{align}
  \label{eq:affine-M}
  ...
  I^y_{i,j} &= \max \begin{cases}
    ...
    I^y_{i,\,j-1} + e.
  \end{cases}
\end{align}$$ The optimal alignment score is $\max(M_{m,n},\, I^x_{m,n},\, I^y_{m,n})$. The traceback now moves between three matrices: a diagonal step in $\mathbf{M}$ represents a match/mismatch, a vertical step in $\mathbf{I}_x$ represents extending or opening a gap in $\mathbf{y}$, and a horizontal step in $\mathbf{I}_y$ represents extending or opening a gap in $\mathbf{x}$. Transitions between matrices correspond to gap openings and closings.

Note that in Equations [\[eq:affine-Ix\]](#eq:affine-Ix){reference-type="eqref" reference="eq:affine-Ix"} and [\[eq:affine-Iy\]](#eq:affine-Iy){reference-type="eqref" reference="eq:affine-Iy"}, the first case pays $g + e$ (opening a new gap and inserting the first character), while the second case pays only $e$ (extending an existing gap by one character). This asymmetry is what distinguishes affine gap penalties from the simpler linear model.

## Semiglobal Alignment {#ch03-pairwise-alignment-sec:semiglobal}
...
The modifications to Needleman--Wunsch are straightforward. The boundary conditions become $$\begin{equation}
  \label{eq:semiglobal-boundary}
  S_{i,0} = 0, \quad S_{0,j} = 0,
\end{equation}$$ so that leading gaps are free. To make trailing gaps free as well, we do not simply take $S_{m,n}$ as the answer. Instead, the optimal score is $$\begin{equation}
  \label{eq:semiglobal-score}
  S^* = \max\!\left(\max_{j} S_{m,j},\; \max_{i} S_{i,n}\right).
\end{equation}$$ We take the maximum over the last row and the last column, then trace back from that cell. Gaps between the traceback endpoint and the corner of the matrix correspond to unaligned tails, which receive no penalty.
...
# Statistical Significance of Alignments {#ch03-pairwise-alignment-sec:blast-statistics}
...
## The Karlin--Altschul Theory {#ch03-pairwise-alignment-sec:karlin-altschul}

**Karlin and Altschul** provided the theoretical foundation in 1990 [@karlin1990]. Their central result concerns *maximal segment pairs* (MSPs): the highest-scoring pair of equal-length segments, one from each sequence, with no gaps.[^ch03-pairwise-alignment-10]

Consider two random sequences of lengths $m$ and $n$, drawn independently from a distribution with amino acid frequencies $p_1, \ldots, p_{20}$. Let $s(a,b)$ be a scoring matrix with the property that $$\begin{equation}
  \label{eq:negative-expected}
  \sum_{a,b} p_a \, p_b \, s(a,b) < 0,
\end{equation}$$ so that the expected score per aligned pair is negative (random alignments tend to have negative scores), and that at least one entry $s(a,b) > 0$ (so that positive-scoring segments can exist). Under these conditions, the score $S$ of the MSP follows a Gumbel extreme value distribution in the limit of long sequences.

The key parameter is $\lambda$, defined as the unique positive solution to $$\begin{equation}
  \label{eq:karlin-lambda}
  \sum_{a=1}^{20} \sum_{b=1}^{20} p_a \, p_b \, e^{\lambda \, s(a,b)} = 1.
\end{equation}$$ This equation always has a unique positive solution when conditions [\[eq:negative-expected\]](#eq:negative-expected){reference-type="eqref" reference="eq:negative-expected"} and the existence of positive scores are satisfied. To see why, define $g(\lambda) = \sum_{a,b} p_a \, p_b \, e^{\lambda \, s(a,b)}$. We have $g(0) = 1$ (trivially, since $e^0 = 1$ and the $p_a$'s sum to one). The derivative $g'(0) = \sum_{a,b} p_a \, p_b \, s(a,b) < 0$ by assumption. And $g(\lambda) \to \infty$ as $\lambda \to \infty$ because at least one $s(a,b) > 0$. So $g$ starts at 1, initially decreases, and eventually grows without bound. By the intermediate value theorem, there is exactly one positive $\lambda$ where $g(\lambda) = 1$ again.

The probability that the MSP score exceeds a threshold $x$ is $$\begin{equation}
  \label{eq:gumbel}
  P(S \geq x) \approx 1 - \exp\!\left(-K m n \, e^{-\lambda x}\right),
\end{equation}$$ where $K$ is a second parameter that depends on $\lambda$, the scoring matrix, and the sequence composition.[^ch03-pairwise-alignment-11] For large $x$, this simplifies to $$\begin{equation}
  \label{eq:gumbel-approx}
  P(S \geq x) \approx K m n \, e^{-\lambda x}.
\end{equation}$$

**The E-value** (expectation value) is the expected number of MSPs with score at least $S$ in a comparison of sequences of lengths $m$ and $n$: $$\begin{equation}
  \label{eq:evalue}
  E = K m n \, e^{-\lambda S}.
\end{equation}$$ An E-value of 0.01 means that we expect to see a score this high 1% of the time by chance. In database searches where we compare a query of length $m$ against a database of total length $N = \sum_k n_k$, the effective search space is $mN$ and the E-value becomes $$\begin{equation}
  \label{eq:evalue-database}
  E = K m N \, e^{-\lambda S}.
\end{equation}$$ The standard threshold for reporting a hit is $E < 0.01$ or $E < 10^{-3}$, depending on the application.

A useful derived quantity is the *bit score*, which normalizes the raw score to be independent of the scoring system: $$\begin{equation}
  \label{eq:bitscore}
  S' = \frac{\lambda S - \ln K}{\ln 2}.
\end{equation}$$ A bit score of 50 corresponds to an E-value of approximately $mN \cdot 2^{-50}$. For a typical database search with $mN \approx 10^{11}$, this gives $E \approx 10^{-4}$, which is a confident hit.
...
## The BLAST Algorithm {#ch03-pairwise-alignment-sec:blast}

**Smith--Waterman** is optimal but slow. For a protein query of length $m = 300$ against a database of $N = 10^{10}$ residues, full dynamic programming requires approximately $3 \times 10^{12}$ operations. Altschul *et al.* introduced BLAST (Basic Local Alignment Search Tool) in 1990 [@altschul1990] as a heuristic that sacrifices a small amount of sensitivity for a dramatic gain in speed. The key idea is to avoid computing most of the dynamic programming matrix by focusing on short exact or near-exact matches that serve as "seeds" for longer alignments.

![BLAST output for a lactotransferrin query: hit list ranked by score and E-value, with identity and coverage bars.](../../../figures/ch03_blast_results.png){#ch03-pairwise-alignment-fig:blast-results width="\\linewidth"}
...
# Chapter Notes {#ch03-pairwise-alignment-sec:ch03-notes}

The history of sequence alignment is intertwined with the history of molecular evolution. Dayhoff's *Atlas of Protein Sequence and Structure* [@dayhoff1978] was as much a contribution to evolutionary biology as to bioinformatics; her PAM matrices encoded an explicit model of the protein evolutionary process calibrated from empirical substitution counts. The intellectual lineage runs from Pauling and Zuckerkandl's 1962 proposal that protein sequences record evolutionary history, through Fitch and Margoliash's 1967 phylogenetic trees built from cytochrome $c$ distances, to Dayhoff's mutation probability matrices. The BLOSUM matrices of Henikoff and Henikoff [@henikoff1992] represented a philosophical shift: from model-based to empirical, from extrapolating close homologs to measuring substitution patterns directly at the distance of interest. Both approaches produce matrices that are broadly similar, and debates about which is "better" have largely been resolved by benchmarking on specific tasks.

The dynamic programming approach to sequence alignment has deep roots in operations research and computer science. Needleman and Wunsch [@needleman1970] were not the first to apply dynamic programming to string comparison (that credit arguably goes to Vintsyuk's 1968 work on speech recognition), but they were the first to apply it to biological sequences. The Smith--Waterman algorithm [@smith1981] adapted the approach for local alignment, and Gotoh's 1982 paper provided the efficient three-matrix formulation for affine gap penalties that we use today. The algorithmic framework is now so standard that it is easy to forget how non-obvious these ideas were at the time. Needleman and Wunsch themselves did not frame their algorithm in the language of dynamic programming; the connection was made explicit by later authors.

Karlin and Altschul's 1990 paper [@karlin1990] on the statistics of ungapped local alignments was a landmark contribution that transformed database searching from an empirical exercise into a rigorous statistical procedure. Before their work, the significance of alignment scores was assessed by ad hoc methods: shuffling sequences, fitting normal distributions, or simply using experience-based cutoffs. The Gumbel distribution result gave the field a principled framework for computing E-values and P-values. The extension of BLAST to gapped alignments [@altschul1997] required empirical calibration rather than exact theory, but the Karlin--Altschul parameters $\lambda$ and $K$ continue to provide the foundation.

BLAST itself [@altschul1990] is perhaps the single most influential algorithm in computational biology, measured by citations, by daily usage, or by scientific impact. Its design illustrates a recurring theme in bioinformatics: exact algorithms exist (Smith--Waterman) but are too slow for the scale of modern databases, so heuristic approximations that sacrifice a small amount of sensitivity for orders-of-magnitude speedups become the practical standard. The success of BLAST rests not only on its algorithmic cleverness (the word-hit seeding strategy) but also on the solid statistical foundation provided by Karlin and Altschul, which allows users to interpret results with confidence.

[^ch03-pairwise-alignment-1]: The first edition of the *Atlas* appeared in 1965 with 65 sequences. By the 1978 edition [@dayhoff1978], it contained over 1,000 sequences organized into 71 superfamilies.

[^ch03-pairwise-alignment-2]: PAM stands for "Point Accepted Mutation." One PAM unit is defined as the amount of evolutionary time in which 1% of amino acid positions are expected to have changed.

[^ch03-pairwise-alignment-3]: This assumes a time-homogeneous Markov chain, which is an approximation. Real proteins evolve at different rates at different sites, and the amino acid composition drifts over time.

[^ch03-pairwise-alignment-4]: Why PAM-250 and not, say, PAM-100? At 250 PAMs, roughly 80% of amino acid positions have changed. This distance is appropriate for detecting remote homologs. For closely related sequences, PAM-120 or lower is more sensitive.

[^ch03-pairwise-alignment-5]: The clustering reduces the influence of closely related sequences. Without it, a block containing 50 nearly identical human hemoglobin sequences and 2 fish hemoglobins would be dominated by human-human comparisons.

[^ch03-pairwise-alignment-6]: For $m = n = 100$, the number of alignments is approximately $9 \times 10^{58}$.

[^ch03-pairwise-alignment-7]: These scores are from the original PAM-250 matrix. Positive values indicate substitutions favored by evolution; negative values indicate disfavored substitutions.

[^ch03-pairwise-alignment-8]: Typical values for protein alignment are $g = -11$ and $e = -1$ in BLAST's default parameterization.

[^ch03-pairwise-alignment-9]: This three-matrix formulation is due to Gotoh (1982). It runs in $O(mn)$ time, the same as the linear-gap version. A naive implementation of affine gaps without the three-matrix trick would require $O(mn \cdot \max(m,n))$ time.

[^ch03-pairwise-alignment-10]: The restriction to ungapped alignments is important. The Karlin--Altschul theory gives exact distributional results only for the ungapped case. The extension to gapped alignments is empirical rather than theoretical, though it works well in practice.

[^ch03-pairwise-alignment-11]: $K$ can be computed from the scoring matrix and the sequence composition, but the formula is more involved than that for $\lambda$. In practice, $K$ is estimated by simulation.

[^ch03-pairwise-alignment-12]: For $r = 0.7$, the optimal ratio is approximately 2.3, meaning the mismatch penalty should be about 2.3 times the match reward. The commonly used $+1/-3$ scoring scheme corresponds to an optimal target identity of about 0.75.

[^ch03-pairwise-alignment-13]: We assume that all three types of mismatch are equally likely, which is an approximation. Transitions (purine $\leftrightarrow$ purine or pyrimidine $\leftrightarrow$ pyrimidine) are more common than transversions in practice.
